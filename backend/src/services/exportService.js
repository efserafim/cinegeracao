/**
 * Exportação de listas: Excel, CSV e PDF.
 */
...
async function obterLinhas(eventoId) {
  const items = await prisma.inscricao.findMany({
    where: { eventoId },
    include: { participante: true, ingresso: true },
...
async function exportarExcel(eventoId) {
  ...
  ws.columns = [
    ...
    { header: 'Ingresso', key: 'ingresso', width: 18 },
  ];
...
async function exportarCsv(eventoId) {
  const linhas = await obterLinhas(eventoId);
  const headers = [
    'codigo', 'nome', 'telefone', 'email', 'paroquia', 'cidade',
    'status', 'valor', 'dataInscricao', 'ingresso',
  ];
...
async function exportarPdf(eventoId) {
  ...
  return new Promise((resolve, reject) => {
    ...
    linhas.forEach((l, idx) => {
      doc.text(
        `${idx + 1}. ${l.nome} | ${l.telefone} | ${l.cidade} | ${l.status} | R$ ${l.valor}`,
      );
    });
...
/**
 * Relatório financeiro resumido do evento.
 */
