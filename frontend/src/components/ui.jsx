export function StatusBadge({ status }) {
  ...
  return <span className={`inline-flex rounded-md px-2 py-0.5 text-xs font-medium ${map[status] || map.CANCELADA}`}>
      {labels[status] || status}
    </span>;
}
export function StatCard({ label, value, hint }) {
  return <div className="animate-fade-up rounded-2xl border border-black/5 bg-white/80 p-4 shadow-sm dark:border-white/10 dark:bg-slate-900/70">
...
export function Button({ children, variant = "primary", className = "", ...props }) {
  const base = "inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold transition disabled:opacity-50";
  const variants = {
    primary: "bg-[var(--color-forest)] text-white shadow-sm shadow-red-900/20 hover:bg-[var(--color-forest-deep)]",
    secondary: "bg-[var(--color-sand-deep)] text-[var(--color-ink)] hover:bg-[#d8dee9] dark:bg-slate-700 dark:text-white",
...
export function Input({ label, error, className = "", ...props }) {
  return <label className="block space-y-1.5">
      {label && <span className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--color-ink-soft)] dark:text-slate-400">
          {label}
        </span>}
      <input
